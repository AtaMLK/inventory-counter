...
export default function HomeScreen() {
  ...
  const [quantityItem, setQuantityItem] = useState<InventoryCountRow | null>(
    null,
  );
  ...
  const refreshItems = useCallback(async (sessionId = session?.id) => {
    if (!sessionId) return;
  ...
  const loadSession = useCallback(async () => {
    try {
      ...
      Alert.alert(
        "Database Error",
        "Could not load the inventory session.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function captureOcr() {
    if (!session || locked || ocrBusy || !cameraReady || !cameraRef.current)
      return;
    ...
    try {
      ...
      for (const token of searchTerms.slice(0, 35)) {
        ...
        for (const product of matches) {
          ...
          if (
            product.productCode.trim().toUpperCase() === token.toUpperCase()
          ) {
      ...
      Alert.alert(
        "OCR Error",
        "Could not read the text. Move closer, keep the label flat, and try again.",
      );
    } finally {
      setOcrBusy(false);
    }
  ...
  const cameraShouldRun =
    cameraActive && (scanMode === "scan" || scanMode === "ocr") && !locked;
      ...
      <View style={styles.cameraSection}>
        <View style={styles.cameraCard}>
          ...
          <View style={styles.cameraStatus}>
            <Text style={styles.cameraStatusText}>
              {scanMode === "ocr"
                ? "OCR • POINT AT PRODUCT LABEL"
                : "BARCODE READY"}
            </Text>
          </View>
          {scanMode === "ocr" ? (
            <TouchableOpacity
              style={styles.ocrCaptureButton}
              onPress={() => void captureOcr()}
              disabled={ocrBusy || !cameraReady}
            >
              {ocrBusy ? (
                <ActivityIndicator color="#071009" />
              ) : (
                <Ionicons name="scan-outline" size={24} color="#071009" />
              )}
        ...
        <View style={styles.modeRow}>
          <TouchableOpacity
            style={[
              styles.modeButton,
              scanMode === "scan" && styles.modeButtonActive,
            ]}
            onPress={() => {
              setScanMode("scan");
              setCameraReady(false);
          ...
          >
            ...
            <Text
              style={[
                styles.modeText,
                scanMode === "scan" && styles.modeTextActive,
              ]}
            >
              SCAN
            </Text>
          ...
          >
            <Ionicons
              ...
              size={24}
              color={scanMode === "ocr" ? "#70e884" : "#e7e7e7"}
            />
            <Text
              style={[
                styles.modeText,
                scanMode === "ocr" && styles.modeTextActive,
              ]}
            >
              OCR
            </Text>
          ...
          <TouchableOpacity
            style={[
              styles.modeButton,
              scanMode === "search" && styles.modeButtonActive,
            ]}
            onPress={openSearch}
            disabled={locked}
          >
            <Ionicons
              name="search-outline"
              size={24}
              color={scanMode === "search" ? "#70e884" : "#e7e7e7"}
            />
            <Text
              style={[
                styles.modeText,
                scanMode === "search" && styles.modeTextActive,
              ]}
            >
              SEARCH
            </Text>
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyText}>
              Scan a barcode, use OCR, or Search to add the first item.
            </Text>
          </View>
        }
        ListFooterComponent={
          <View style={styles.footer}>
            ...
            <TouchableOpacity
              style={[
                ...
                locked && styles.completeButtonDisabled,
              ]}
            ...
            >
              <Ionicons
                name={
                  locked ? "lock-closed-outline" : "checkmark-circle-outline"
                }
                size={21}
              ...
              <Text
                style={[
                  ...
                  locked && styles.completeButtonTextDisabled,
                ]}
              >
        ...
        <View style={styles.ocrBackdrop}>
          <View style={styles.ocrSheet}>
            <View style={styles.ocrHeader}>
              <View style={{ flex: 1 }}>
                <Text style={styles.modalTitle}>OCR Result</Text>
                <Text style={styles.modalSubtitle}>
                  Select a product if OCR could not make an exact code match.
                </Text>
              </View>
              <TouchableOpacity
                style={styles.closeButton}
                onPress={() => setOcrVisible(false)}
              >
                <Ionicons name="close" size={24} color="#f2f2f2" />
              </TouchableOpacity>
            </View>
            <View style={styles.ocrTextBox}>
              <Text style={styles.ocrLabel}>READ TEXT</Text>
            ...
            {ocrResults.length === 0 ? (
              <View style={styles.searchEmpty}>
                <Ionicons name="alert-circle-outline" size={30} color="#777" />
                <Text style={styles.searchEmptyText}>
                  No matching product found. Try again closer to the label.
                </Text>
              </View>
            ) : (
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity
                    ...
                    onPress={() => void addOcrProduct(item)}
                  >
                    ...
                    <View style={styles.addMatch}>
                      <Ionicons name="add" size={20} color="#071009" />
                      <Text style={styles.addMatchText}>+1</Text>
                    </View>
                  </TouchableOpacity>
                )}
              />
            ...
            <TouchableOpacity
              ...
              onPress={() => setOcrVisible(false)}
            >
              <Ionicons name="camera-outline" size={20} color="#e7e7e7" />
              <Text style={styles.retryOcrText}>BACK TO CAMERA</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
            ...
            {searchLoading ? (
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity
                    style={styles.searchResult}
                    onPress={() => void addProductFromSearch(item)}
                  >
                    <View style={styles.searchResultInfo}>
                      <Text style={styles.searchResultName} numberOfLines={1}>
                        {item.productName}
